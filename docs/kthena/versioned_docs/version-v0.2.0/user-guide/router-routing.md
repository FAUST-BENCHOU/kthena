...
The router supports various routing strategies, from simple model-based forwarding to complex weighted distribution, header-based routing, and PD (Prefill/Decode) disaggregation. This flexibility allows for advanced deployment patterns including canary releases, A/B testing, load balancing across heterogeneous model deployments, and PD-separated inference for lower latency and better resource utilization.
...
### 5. PD Disaggregation Routing

**Scenario**: Route inference requests to Prefill/Decode disaggregated backends, where P-instances handle prefill and D-instances handle decode stages independently.

**Traffic Processing**: When a request arrives for a PD-disaggregated model, the router selects matched P and D pods via `pdGroup` labels, proxies prefill to the P-instance and decode to the D-instance, coordinating KV state exchange between them.
...
3. Via `pdGroup`, router selects a P-pod for prefill and a D-pod for decode (same `groupKey` value)
4. Prefill runs on P-instance; decode runs on D-instance, with KV state exchanged between them (configure `kvConnector` for nixl/mooncake when needed)
